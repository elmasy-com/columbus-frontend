<script>
	...
	async function isValid() {
		// A hack to use "let promise = isValid();" without real API call.
		if (domain === '') {
			return;
		}

		const res = await fetch(`https://columbus.elmasy.com/api/tools/isvalid/` + domain);
...
<div id="result">
	...
	{:then valid}
		...
		{#if domain != '' && valid != undefined}
			<p>Valid: <code>{valid.result}</code></p>
		{/if}
	{:catch error}
