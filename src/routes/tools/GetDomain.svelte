<script>
	...
	async function getDomain() {
		// A hack to use "let promise = getDomain();" without real API call.
		if (domain === '') {
			return;
		}

		const res = await fetch(`https://columbus.elmasy.com/api/tools/domain/` + domain);
...
<div id="result">
	...
	{:then result}
		...
		{#if domain != '' && result != undefined}
			<p>Domain: <code>{result.result}</code></p>
		{/if}
	{:catch error}
